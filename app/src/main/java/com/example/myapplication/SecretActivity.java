...
public class SecretActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        StatusUtils.initToolbar(SecretActivity.this, title, true, false);
        initView();
    }
    ...
    private void findPwd() {
        ...
        if(TextUtils.isEmpty(username)) {
            Toast.makeText(SecretActivity.this, "用户名不能为空", Toast.LENGTH_SHORT).show();
        } else if(!SharedUtils.isExist(this, username)) {
            Toast.makeText(SecretActivity.this, "输入的用户名不存在", Toast.LENGTH_SHORT).show();
        } else if(TextUtils.isEmpty(validateName)) {
            Toast.makeText(SecretActivity.this, "验证名不能为空", Toast.LENGTH_SHORT).show();
        } else if(!validateName.equals(security)) {
            Toast.makeText(SecretActivity.this, "输入的密保不正确", Toast.LENGTH_SHORT).show();
        } else {
            tvResetPwd.setVisibility(View.VISIBLE);
    ...
    private void setSecurity() {
        if(TextUtils.isEmpty(validateName)) {
            ...
            Toast.makeText(SecretActivity.this, "密保设置成功", Toast.LENGTH_SHORT).show();
